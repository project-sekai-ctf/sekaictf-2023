...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    // derive some keys that weren't explicitly shared with us
    let rich_boi_bid = Pubkey::find_program_address(&[auction.as_ref(), rich_boi.as_ref()], &chall_id).0;
    let user_bid     = Pubkey::find_program_address(&[auction.as_ref(), user.as_ref()], &chall_id).0;
    ...
    println!("rich_boi_bid : {}", rich_boi_bid);
    println!("user_bid     : {}", user_bid);
    println!("");
