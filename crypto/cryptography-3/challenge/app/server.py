...
from SECRET import prev_flag, flag, generate_graph, decrypt
...
if __name__ == '__main__':
    if input("Flag for cryptoGRAPHy 2: ").strip() != prev_flag:
        print("[!] Wrong flag!")
        exit()
    try:
        G = generate_graph()
        ...
        print("[!] In each query, input the shortest path decrypted from response. It will be a string of space-separated nodes from source to destination, e.g. '1 2 3 4'.")
        for q in range(10):
